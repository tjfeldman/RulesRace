...
func _should_use_group_rule():
	...
	"""	
	var action_cost = GroupRules.get_trigger();
	var will_pay_cost = action_cost in [GroupRules.Trigger.DISCARD_TICKET, GroupRules.Trigger.FORFEIT_DIE];
	if !will_pay_cost: return false; #exit early if we won't pay the cost
	
	var action_effect = GroupRules.get_effect();
	var wants_effect = false;
	match action_effect:
		GroupRules.Effect.MOVE_ONE:
			#Moving 1 is only valuable if it costs a ticket and we have more than 1.
			wants_effect = action_cost == GroupRules.Trigger.DISCARD_TICKET and self._escapeTickets > 1;
		GroupRules.Effect.GAIN_TICKET:
			#Only valuable if we have no escape tickets and the cost is forfeiting die
			wants_effect = action_cost == GroupRules.Trigger.FORFEIT_DIE and self._escapeTickets == 0;
		...
		GroupRules.Effect.SEND_PLAYER_BACK_ONE:
			...
			if wants_effect:
				...
				wants_effect = GroupRules.Trigger.DISCARD_TICKET and self._escapeTickets > 1;
...
func selectOfficeReward():
	...
	"""
	if !GroupRules.is_group_rule_active():
...
func selectTargetPlayer(playerlist: Array[Player], is_can_rule):
	#Bot Player waits a bit for human players
	await get_tree().create_timer(0.5).timeout;
	if is_can_rule and GroupRules.HARMFUL_EFFECTS.has(GroupRules.get_effect()):
